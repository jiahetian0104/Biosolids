...
## this is to get rid of PFDA concentrations
temporal_data <- temporal_data %>%
  dplyr::select(-12, -13, -14, -15)
...
write.csv(INT,"2022_dose_PFOS.csv") #save dose output file

#######################################################################################
############################PFOS PopTK MODEL########################################
#### Single Compartment PK Model for Cattle uptake/elimination of PFOS with population model for stochastic simulations 
### Authors: Antti Mikkonen (UniSA/EPA VIC) and Barbara Astmann (MECDC)
## Version: 1.4
...
means <- params %>% filter(Livestock == "Cattle", Analyte == "PFOS") %>% 
...
CV <- params %>% filter(Livestock == "Cattle", Analyte == "PFOS") %>%
...
## Ledgerock
int <- read.csv("2022_dose_PFOS.csv") #Insert file path for dose output file generated at line 125
...
write.csv(summary, "summary_PFOS.csv") 
...
labs(title = "PFOS distribution in cattle - 2022", 
      x = "Time (years)",
      y = "PFOS ng/mL or ng/g")
